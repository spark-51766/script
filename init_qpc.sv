...
task init_qpc(
    ...
    hca_qpc.qpc_seg5.txwqe_sq_send_npsn = hca_qpc.qpc_seg0.send_start_psn;  // 发送请求psn号
    ...
    hca_qpc.qpc_seg5.txwqe_sq_wqe_start_psn = hca_qpc.qpc_seg0.send_start_psn;  // WQE起始PSN，重传阶段保持不变。
    ...
    // ── seg 06 ──
    hca_qpc.qpc_seg6.txwqe_rsp_newest_psn = hca_qpc.qpc_seg0.rcv_start_psn - 1;  // resp/ack/nak调度完成的最新psn号， 非乱序时，txt_resp_newest_psn != rxt_resp_newest_psn时，表示有新的ack/nak/response未发送。 乱序时，txt_resp_newest_psn != (rxt_resp_left_psn-1)时，表示有新的ack/nak/response未发送。 veroce下复用为read/atomic response发送占用的psn号，处理方式和sq的处理方式保持一致
    ...
    hca_qpc.qpc_seg6.txwqe_rsp_bk_psn = hca_qpc.qpc_seg0.rcv_start_psn - 1;  // read rsp存储的断点信息中的psn号，在获取rsp cache时，初始psn需要从rsp cache中获取，执行rsp cache sge中间需要从该处获取发送psn号; [23:0]veroce下为T端ack响应的psn号
    ...
    hca_qpc.qpc_seg8.txeng_npsn = hca_qpc.qpc_seg0.send_start_psn;  // TX_ENG计算的下一个请求包的PSN，仅在SQ请求时更新，重传时会回退，
    ...
    hca_qpc.qpc_seg8.txeng_newest_ack_req_psn_l = hca_qpc.qpc_seg0.send_start_psn - 1;  // 最新置位ACK_REQ的请求包PSN低16位，仅在RC SQ请求时更新，重传时不回退
    hca_qpc.qpc_seg8.txeng_newest_ack_req_psn_h = hca_qpc.qpc_seg0.send_start_psn - 1;  // 最新置位ACK_REQ的请求包PSN低16位，仅在RC SQ请求时更新，重传时不回退
    ...
    // ── seg 09 ──
    hca_qpc.qpc_seg9.rxi_upsn = hca_qpc.qpc_seg0.send_start_psn;  // rxi最老未应答psn；乱序接收和选择性重传时，代表未应答窗口的左边界
    ...
    hca_qpc.qpc_seg10.rxi_unack_wqe_start_psn = hca_qpc.qpc_seg0.send_start_psn;  // 用于记录每次最开始未确认的WQE的start_psn，用于重传时通知TX需要重传的起点，可以计算go back to 0/n
    ...
    hca_qpc.qpc_seg10.rxi_sq_oldest_unack_psn = hca_qpc.qpc_seg0.send_start_psn;  // rxi最旧未应答的psn；sqd2rts 时 txwqe 会同步该irrl ptr
    ...
    hca_qpc.qpc_seg11.rxt_epsn = hca_qpc.qpc_seg0.rcv_start_psn;  // 非乱序接收模式时，表示预期收到包的PSN 乱序接收模式时，表示顺序收包的预期PSN，即上一个包的PSN+1，用于乱序收包DFX
    ...
    hca_qpc.qpc_seg11.rxt_left_psn = hca_qpc.qpc_seg0.rcv_start_psn;  // 乱序接收时，接收有效请求的左边界
    hca_qpc.qpc_seg11.rxt_right_psn = hca_qpc.qpc_seg0.rcv_start_psn - 1;  // 乱序接收时，接收有效请求的右边界
    ...
    hca_qpc.qpc_seg11.rxt_dif_dmalen = veroce_en ? {8'b0,hca_qpc.qpc_seg0.rcv_start_psn-1} : '0;  // DIF模式下的待收包长度，用于校验message总长度 veRoCE场景： [23:0]：复用为rxt_sack_right_psn，接收请求达到阈值或者TMO，SACK的右边界 [31:24]：复用为rxt_sack_send_cnt，接收请求达到阈值或者TMO，触发SACK门铃时cnt++
    ...
    hca_qpc.qpc_seg12.rxt_resp_newest_psn = hca_qpc.qpc_seg0.rcv_start_psn - 1;  // 需要调度响应包的最新psn veRoCE场景：接收请求的最新psn
    ...
    hca_qpc.qpc_seg12.rxt_tmo_dctn = hca_qpc.qpc_seg0.rcv_start_psn;  // DCR超时对应的DCTN veRoCE场景：复用为rxt_rsp_left_psn，接收ack_rsp/sack_rsp的左边界，即最早未应答的rsp_upsn
    ...
    hca_qpc.qpc_seg12.rxt_tmo_sqpn = hca_qpc.qpc_seg0.rcv_start_psn - 1;  // DCR超时对应的SQPN veRoCE场景：复用为rxt_rsp_newest_psn，需要调度rsp的最新psn，用于写resp cache的起始psn
    ...
    hca_qpc.qpc_seg12.rxt_read_max_psn = hca_qpc.qpc_seg0.rcv_start_psn - 1;  // 乱序接收时，read请求右边界 veRoCE场景：复用为rxt_rmsn_last_psn，记录已应答MSG的last_psn
